...
public class Day05 implements Day<Day05.Almanac, Long> {
    ...
    public Almanac cookInput(String rawInput) {
        List<String> chunks = new ArrayList<>(Arrays.asList(InputParser.toChunks(rawInput)));
    ...
    private MappingGroup[] parseMappingGroups(List<String> chunks) {
        return chunks.stream().map(chunk -> {
            MappingEntry[] mappingEntries = Arrays.stream(InputParser.toLines(chunk))
    ...
    @Override
    public Long solvePart1(Almanac almanac) {
        long result = Long.MAX_VALUE;

        for (long value : almanac.seeds()) {
            for (MappingGroup mappingGroup : almanac.mappingGroups()) {
    ...
    @Override
    public Long solvePart2(Almanac almanac) {
        long result = Long.MAX_VALUE;

        for (int i = 0; i < almanac.seeds().length; i += 2) {
            long from = almanac.seeds[i];
            long to = from + almanac.seeds[i + 1] - 1;
